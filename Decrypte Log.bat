...
REM  Productname: DeepCommun Chat Log Decryptor
REM  Filedescription: Decryptor Log
REM  Copyrights: C 420
REM  Trademarks: MLG Rekt
REM  Originalname: decryptage.bat
REM  Comments: Réservé à l'admin du DeepCommun
REM  Productversion:  0. 0. 2. 0
REM  Fileversion:  0. 0. 1.23
REM  Internalname: decrypt.exe
REM  Appicon: ..\..\..\Pictures\trucage photo\md5decrypter.png,qecb=63-0.pagespeed.ce.nor_DV0QCR.ico
...
echo Clee invalide...
 echo >> faux.txt
 attrib -H faux.txt
echo %time% %username% >> faux.txt
 attrib +H faux.txt
:pass
set /p pass= Clee de decryptage:
if "%pass%"=="seoulhack" goto juste
if not "%pass%"=="seoulhack" goto faux
:juste
cls
echo Code Bon
color a
...
Set /p save= Voulez vous sauvegarder ?[y/n]:
if "%save%"=="y" goto save
if "%save%"=="n" goto nosave
:save
